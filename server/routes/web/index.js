module.exports = app =>{
    ...
    router.get("/news/list",async(req,res)=>{
        ...
        const cats = await Category.aggregate([ //聚合查询
            {$match:{parent:parent._id}},   // 条件查询，第一个为字段，第二个为新闻分类的id，对应关系型数据库的where
            // 找出 新闻分类 的 下属分类
            {$lookup:{      // 以它为主体查询另外一个集合，对应join
                from:'articles', //关联哪个集合
                localField:'_id', //本地键，根据 下属分类 的 哪个 值去查
                foreignField:'categories', //外地键，去关联到的集合中的 哪里 查
                as:'newsList' //查到的结果的名字
            }},
            // 在文章集合中的categories字段中找出符合下属分类id的数据，结果命名为newsList
            {
                $addFields:{ //添加字段，也可以对已有字段进行修改
                    newsList:{$slice:['$newsList',5]} // newsList字段，$slice方法的第一个值是之前的原有字段，第二个是要取几个
                }
            }
        ])
        // console.log(cats)// cats是新闻分类的四个下属分类，带有newsList
        const subCats = cats.map( v=> v._id)
        // console.log(subCats) // 四个下属分类的id
        cats.unshift({ //前插入一个新的对象
            name:'热门', 
            newsList:await Article.find().where({ 
                categories:{$in:subCats} //查询文章集合中所有id与下属分类id相符的文章
            }).populate('categories').limit(5).lean() //只要五个，lean表示返回纯json字符，不加的话嵌套的对象会变成[object]
        })
        cats.map( cat =>{
    ...
    router.get("/herosdetail/:id",async(req,res)=>{
